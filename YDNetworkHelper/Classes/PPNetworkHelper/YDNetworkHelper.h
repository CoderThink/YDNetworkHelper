...
/// 请求成功的Block
typedef void(^YDHttpRequestSuccess)(id responseObject);

/// 请求失败的Block
typedef void(^YDHttpRequestFailed)(NSError *error);

/// 缓存的Block
typedef void(^YDHttpRequestCache)(id responseCache);

/// 上传或者下载的进度, Progress.completedUnitCount:当前大小 - Progress.totalUnitCount:总大小
typedef void (^YDHttpProgress)(NSProgress *progress);
 ...
 * 内修改,再加上此封装在支持CocoaPods后,如果使用者pod update最新YDNetworkHelper,那又要重新修改此
 * 封装内的相关参数.
 *
 ...
 * 了,大大降低了项目的后期维护难度
 *
 * 综上所述,最终还是将设置参数的接口暴露出来,如果通过CocoaPods方式使用YDNetworkHelper,在设置项目网络
